...
#for prei in range(n_item):
    trainset=read_csv('prePredict',header=None)
    trainsetorg = read_csv('prePredict.csv',header=None)
    traindata_all=trainset.values
    n_points = traindata_all.shape[0]
    n_item = traindata_all.shape[1]
    for prei in range(n_item):
    ...
    np.savetxt('prediction3.csv', prediction, delimiter=",", fmt = '%s')
    trainsetorg.to_csv('data3.csv',index =None,header =None)
    ...
    DEC=trainsetorg.values[:,0]*100
    x=np.arange(1,DEC.size+1)
    # f1=np.polyfit(x,DEC,2)
    # p1=np.poly1d(f1)
    # DEC_trend=p1(DEC)
    A1,B1=optimize.curve_fit(f1,x,DEC)[0]
    DEC_trend=A1*x+B1#**2+B1*x+C1
    #if DEC_trend[-1]-DEC_trend[0]>2.5 :  #效率降级大于2%
    if DEC[-1]>2:
        plot1=plt.plot(x,DEC_trend,'r')
        plot2=plt.plot(x,DEC,'s')
        plt.show()
        break
# for nlp in range(maxloops):
